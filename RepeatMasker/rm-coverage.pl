#!/usr/bin/perl
# take rm .out file and summarizes famillies and classes
...
my %element; # holds the class and family as key and the coverage as value
...
while (my $line = <RM>) {
	...
	#update the hash
	my $coverage = abs($b2 - $b1);
	$element{"$sf\t$fam"} += $coverage;
}

#Print hash
foreach my $name (keys %element) {
